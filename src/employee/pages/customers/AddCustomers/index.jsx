import { Button, MenuItem, TextField, useMediaQuery, Snackbar, Alert } from '@mui/material';
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      await createAccount(dataUpload)
        ...
        .then((data) => {
          console.log({ data });
          setAddStatus({ message: 'Add success', severity: 'success' });
        })
        .catch((error) => {
      ...
      if (!err?.status) {
        ...
        console.log(err);
        setError('Thêm mới không thành công');
      }
    }
  };
